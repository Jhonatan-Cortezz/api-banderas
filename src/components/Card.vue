<template>
  <div class="card">
    <div class="card-body">
      ...
      <p class="text-center">
        <img :src="pais.flag" class="img-fluid w-50" />
      </p>
      <p class="card-text">
        <span class="badge bg-dark d-block mb-1">
          Native name: {{ pais.nativeName }}
        </span>
        <span class="badge bg-info p-3 d-block mb-1">
          Population: {{ numeroFormato(pais.population) }}
        </span>
        <span class="badge bg-dark d-block mb-1"
          >Capital: {{ pais.capital }}</span
        >
        <span class="badge bg-dark d-block mb-1"
          >Region: {{ pais.region }}</span
        >
      </p>
    </div>
  </div>
